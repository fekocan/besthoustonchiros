...
export async function generateMetadata({ params }: TherapistPageProps): Promise<Metadata> {
  ...
  return {
    title: `${therapist.first_name} ${therapist.last_name} - ${city?.name || 'Houston'} - Best Houstun Chiros`,
      ...
      <main className="flex-grow bg-slate-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {city && (
            <Link
              href={`/city/${city.slug}`}
              className="inline-flex items-center text-slate-600 hover:text-blue-600 mb-6 transition-colors font-medium"
            >
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back to {city.name} chiropractors
            </Link>
          )}
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 md:p-8">
              <div className="text-center mb-6">
                ...
                <p className="text-lg text-slate-600 mb-3">Licensed Chiropractor</p>
        ...
        {city && (
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-8">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
              <h2 className="text-2xl font-bold text-slate-900 mb-4">
                Looking for more chiropractors in {city.name}?
              </h2>
              <p className="text-slate-600 mb-6">
                Browse our directory of licensed chiropractors
              </p>
              <Link
                href={`/city/${city.slug}`}
              ...
              >
                View All {city.name} Chiropractors
              </Link>
            </div>
          </div>
        )}
