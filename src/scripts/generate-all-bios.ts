...
async function generateAllBios() {
    console.log('Fetching chiropractors...');
    ...
    console.log(`Found ${therapists.length} chiropractors. Starting generation...`);
    ...
    for (const therapist of therapists) {
        ...
        Write a profile for a chiropractor named "${fullName}" located in "${cityName}".
        ...
        2. "bio": A detailed, welcoming professional biography (approx 150-200 words). Focus on their expertise in ${(therapist.services || [])[0] || "chiropractic care"} and commitment to patient health in ${cityName}.
